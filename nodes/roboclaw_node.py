...
class ReerWheel:
	...
	def cmd_vel_callback(self, twist):
		if not self.paused:
			...
			Vr_ticks = int((Vr * self.TICKS_PER_METER)/8) # ticks/s
			Vl_ticks = int((Vl * self.TICKS_PER_METER)/8)
			print(Vr_ticks, Vl_ticks)	
...
class SteeringWheel:
	...
	def cmd_vel_callback(self, twist):
		if not self.paused:
			...
			Vr_ticks = int((Vr * self.TICKS_PER_METER)/2) # ticks/s
			Vl_ticks = int((Vl * self.TICKS_PER_METER)/2)
			print(Vr_ticks, Vl_ticks)
			self.roboclaw.setSpeed(Vl_ticks,Vr_ticks)
